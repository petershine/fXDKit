...
- (BOOL)canAnimateWithTransitionSegue:(FXDStoryboardSegue*)transitionSegue {
	...
	UIViewController *destinationScene = transitionSegue.destinationViewController;
	UIViewController *sourceScene = transitionSegue.sourceViewController;
	
	FXDLogObject(destinationScene);
	FXDLogObject(sourceScene);
	
	if ([sourceScene isKindOfClass:[UIViewController class]]
		&& [destinationScene isKindOfClass:[UIViewController class]]) {
		canAnimate = YES;
	}
...
#pragma mark - Public
- (void)coverWithSegue:(FXDStoryboardSegue*)coveringSegue {	FXDLog_DEFAULT;
	...
	UIViewController *presentedScene = coveringSegue.destinationViewController;
	[self addChildViewController:presentedScene];

	
	CGPoint coveringOffset = [self coveringOffsetWithDirectionType:presentedScene.coverDirectionType];
	CGPoint coveringDirection = [self coveringDirectionWithDirectionType:presentedScene.coverDirectionType];
	...
	UIViewController *pushedScene = nil;
...
- (void)uncoverWithSegue:(FXDStoryboardSegue*)uncoveringSegue {	FXDLog_DEFAULT;
	...
	UIViewController *dismissedScene = uncoveringSegue.sourceViewController;
	
	CGPoint uncoveringOffset = [self coveringOffsetWithDirectionType:dismissedScene.coverDirectionType];
	CGPoint uncoveringDirection = [self coveringDirectionWithDirectionType:dismissedScene.coverDirectionType];
	...
	UIViewController *pulledScene = nil;
...
- (void)uncoverAllSceneWithFinishCallback:(FXDcallbackFinish)callback {
	...
	for (UIViewController *childScene in lateAddedSceneArray) {
	...
	UIViewController *rootScene = (self.childViewControllers).firstObject;
